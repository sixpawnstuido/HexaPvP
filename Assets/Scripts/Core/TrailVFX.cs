    ...
    public void TrailMotion(PlayerType playerType,int hexagonElementAmount,Color hexagonColor,int comboStage=1,int trailCurveIndex=0)
    {
        ...
        hexagonSprite.transform.DOMove(avatarTarget.transform.position, .55f)
            ...
            .OnComplete(() =>
            {
               ...
               if(!UIManager.Instance.failedPanel.gameObject.activeInHierarchy || !UIManager.Instance.nextLevelPanel.gameObject.activeInHierarchy) BarController.Instance.ChangeProgress(playerType,hexagonElementAmount);
