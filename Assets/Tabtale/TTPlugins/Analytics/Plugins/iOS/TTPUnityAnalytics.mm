...
extern "C" {
    ...
    const char * ttpGetAdditionalParams()
    {
        ...
        if(analyticsService != nil){
            NSError *error;
            NSDictionary *additionalEventParamsDict = [analyticsService getAdditionalEventParams];
            if(additionalEventParamsDict != nil){
                NSData *jsonData = [NSJSONSerialization dataWithJSONObject:additionalEventParamsDict
                                                                   options:NSJSONWritingPrettyPrinted // Pass 0 if you don't care about the readability of the generated string
                                                                     error:&error];
